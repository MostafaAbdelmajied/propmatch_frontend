...
export function ContractGenerator() {
  ...
  const prefill = useLeaseContractPrefill(
    role === "landlord" && !existing.data ? matchConnectionId : "",
  );
  ...
  if (!matchConnectionId)
    return (
      <EmptyState
        ...
        description="عقد الإيجار يُنشأ من محادثة صفقة متصلة بين المالك والمستأجر."
      />
    );
  if (existing.isLoading || (!existing.data && prefill.isLoading))
    return (
  ...
  if (existing.isError || (!existing.data && role === "landlord" && prefill.isError))
    return (
      <ErrorState
        onRetry={() => {
          existing.refetch();
  ...
  const save = (payload: DraftPayload) =>
    saveDraft.mutate(
      {
  ...
  return (
    <div className="flex flex-col gap-3">
      <ContractBuilder
        ...
        onSendForReview={save}
        onApprove={() => existing.data && router.push(`/contracts/${existing.data.id}`)}
        onReject={() => undefined}
        sending={saveDraft.isPending}
        approving={false}
        rejecting={false}
      />
      {saveDraft.isError && (
        <p className="mx-auto max-w-3xl text-end text-small text-error" role="alert">
          تعذر حفظ المسودة. حاول مرة أخرى.
        </p>
      )}
    </div>
  );

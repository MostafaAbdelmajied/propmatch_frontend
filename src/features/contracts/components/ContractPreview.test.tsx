...
const contract = (overrides: Record<string, unknown> = {}) => ({
  id: "contract-id", matchConnectionId: "match-id", status: "drafting", ownerName: "Owner", tenantName: "Tenant", propertyAddress: "Address", rentAmount: 12000,
  startDate: "2026-08-01", endDate: "2027-07-31", customClauses: [], createdAt: "2026-07-01", changeRequestNote: null,
  witness1Name: null, witness2Name: null, pdfUrl: null, tenantReviewStatus: "PENDING_REVIEW", draftRevision: 2, tenantReviewedRevision: null,
  tenantChangeRequest: null, tenantChangeRequestedAt: null, tenantReviewConfirmedAt: null, canEdit: false, canRequestChanges: false, canConfirmReview: false, canDownloadPdf: true,
  ...overrides,
});
...
describe("ContractPreview review permissions", () => {
  it("shows tenant pending-review actions while keeping the saved contract read-only", () => {
    render(<ContractPreview contract={contract({ canRequestChanges: true, canConfirmReview: true }) as any} />);
  ...
  it("shows tenant waiting/confirmed messages without review actions", () => {
    const { rerender } = render(<ContractPreview contract={contract({ tenantReviewStatus: "CHANGES_REQUESTED" }) as any} />);
    expect(screen.getByText(/طلب التعديل اتبعت للمالك/)).toBeTruthy();
    expect(screen.queryByText("طلب تعديل")).toBeNull();
    rerender(<ContractPreview contract={contract({ tenantReviewStatus: "REVIEW_CONFIRMED", canEdit: false }) as any} />);
  ...
  it("keeps a landlord editable after requested changes and locks after confirmation", () => {
    ...
    const { rerender } = render(<ContractPreview contract={contract({ tenantReviewStatus: "CHANGES_REQUESTED", tenantChangeRequest: "Change date", canEdit: true }) as any} />);
